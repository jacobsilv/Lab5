...
htable_init(htable *ht, int sz, int allow_resize) {
	...
	ht->lock = (pthread_mutex_t*)malloc(sizeof(pthread_mutex_t) * (ht->size));
	memset(ht->lock, 0, sizeof(pthread_mutex_t)*ht->size);
	for (int i =0; i< ht->size; i++){
		pthread_mutex_init(ht->lock+i, NULL);
	}
	bzero(ht->store, sizeof(node *)*ht->size);
	
	ht->rwl = (rwl*)malloc(sizeof(rwl));
	rwl_init(ht->rwl);
...
htable_destroy(htable *ht) {
	...
	for (int i =0 ; i < ht->size; i++){
		pthread_mutex_destroy(ht->lock+i);
	}
...
htable_resize(htable *ht) {
	...
	ht->lock = realloc(ht->lock, sizeof(pthread_mutex_t)*new_size);
...
htable_insert(htable *ht, char *key, void *val) {
	...
	while (curr) {
		if ((curr->hashcode == hcode ) && (strcmp(curr->key, key) == 0)) {
			pthread_mutex_unlock(ht->lock+slot);
			rwl_runlock(ht->rwl);
